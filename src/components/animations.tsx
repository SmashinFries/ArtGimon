...
import { Platform, StyleProp, StyleSheet, TextStyle, View, ViewProps } from 'react-native';
...
type FadeViewProps = ViewProps & {
	disableTransAnim?: boolean;
};
export const FadeView = (props: FadeViewProps) => {
	return (
		<Animated.View
			{...props}
			entering={
				!props.disableTransAnim
					? FadeIn.delay(IS_WEB ? 500 : 200).duration(1000)
					: undefined
			}
			exiting={!props.disableTransAnim ? FadeOut.duration(1000) : undefined}>
			{props.children}
		</Animated.View>
	);
};

export const FadeViewMemo = memo(FadeView);
