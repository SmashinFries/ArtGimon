...
export const useMenuBackground = () => {
    ...
    const loadImages = async () => {
        const firstBatch = await getFakeImage({
            limit: 25,
            tags: '1girl, order:random, rating:g,s, age:<6month',
        });
        const secondBatch = await getRealImage({
            limit: 25,
            tags: '1girl, order:random, rating:g,s, age:<6month',
        });
        if (firstBatch && secondBatch) {
            ...
            // shuffle the images
            const shuffled = combinedImages.reduce(
